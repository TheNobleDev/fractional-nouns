...
/// @title A contract used to convert multi-part RLE compressed images to SVGs with whitespace

/// SVGRendererFragment.sol is a modified version of Nouns' SVGRenderer.sol:
/// https://github.com/nounsDAO/nouns-monorepo/blob/619faf732cb1cccd819791378e4ba90baca05feb/packages/nouns-contracts/contracts/SVGRenderer.sol
/// The only modification is to the height of rendered rectangles, changing it from 10 to 7.

pragma solidity ^0.8.20;

import { ISVGRenderer } from './external/nouns/interfaces/ISVGRenderer.sol';

contract SVGRendererFragment is ISVGRenderer {
    ...
    function _getChunk(uint256 cursor, string[16] memory buffer) private pure returns (string memory) {
        ...
        for (uint256 i = 0; i < cursor; i += 4) {
            chunk = string(
                abi.encodePacked(
                    chunk,
                    '<rect width="', buffer[i], '" height="7" x="', buffer[i + 1], '" y="', buffer[i + 2], '" fill="#', buffer[i + 3], '" />'
                )
            );
        }
        return chunk;
